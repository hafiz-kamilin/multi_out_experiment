...
# define the input
input1 = tf.keras.layers.Input(shape = (columns_num,))
# define the layers
x = tf.keras.layers.Dense(25, activation = "relu")(input1)
# define the middle layer
deep1 = tf.keras.layers.Dense(5, activation = "relu")(x)
deep2 = tf.keras.layers.Dense(5, activation = "relu")(x)
# define the outputs
output1 = tf.keras.layers.Dense(1, activation = "linear")(deep1)
output2 = tf.keras.layers.Dense(1, activation = "linear")(deep2)
# define the model
...
model.fit(train_X, [train_y_x, train_y_y], validation_split = 0.2, epochs = 100, callbacks = [early_stopping_monitor])
